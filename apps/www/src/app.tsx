...
import ChartEvents from "./demos/chart-events";
import chartEventsCode from "./demos/chart-events?raw";
...
import CustomShapeLine from "./demos/custom-shape-line";
import customShapeLineCode from "./demos/custom-shape-line?raw";
...
import legendHoverCode from "./demos/legend-hover?raw";
import Negative from "./demos/negative";
import negativeCode from "./demos/negative?raw";
import PerSeriesData from "./demos/per-series-data";
import perSeriesDataCode from "./demos/per-series-data?raw";
import PercentArea from "./demos/percent-area";
...
import referenceLineSegmentCode from "./demos/reference-line-segment?raw";
...
import VerticalLine from "./demos/vertical-line";
import verticalLineCode from "./demos/vertical-line?raw";
...
const DEMOS: Demo[] = [
  ...
  },
  {
    ...
    desc: "Axis `<AxisLabel>` can force every tick, stagger crowded labels, and render swatches, images, or styled markup per tick.",
    Comp: CustomLabels,
...
const FEATURE_PARITY_DEMOS: Demo[] = [
  {
    id: "vertical-line",
    group: "Layout",
    title: "Vertical line chart",
    desc: 'Covers the Recharts `layout="vertical"` parity gap. Categories render on the Y axis, values on the X axis — a horizontal line orientation for comparing categories side-by-side.',
    Comp: VerticalLine,
    code: verticalLineCode,
  },
  {
    id: "axis-formatting",
    group: "Axis",
    title: "Axis tick formatting",
    desc: "Covers the Recharts `tickFormatter` parity gap. Set `tickFormatter` on `<Axis>` once and default `<AxisLabel>` children render the formatted values.",
    Comp: AxisFormatting,
    code: axisFormattingCode,
  },
  {
    id: "axis-padding",
    group: "Axis",
  ...
  },
  {
    ...
    group: "Reference",
    title: "Compare two lines segment",
    desc: "Covers the Recharts Compare Two Lines use of `ReferenceLine segment`: on hover, connect the active portfolio and comparison values with a data-space segment.",
    Comp: ReferenceLineSegment,
    code: referenceLineSegmentCode,
  },
  {
    id: "legend-hover",
    group: "Legend",
    title: "Legend hover events",
    desc: "Covers the Recharts `onMouseEnter`/`onMouseLeave` parity gap on `<Legend>`. Hover a legend item to highlight the corresponding series.",
    Comp: LegendHover,
    code: legendHoverCode,
  },
  {
    id: "ranged-area",
    group: "Area",
    title: "Ranged area",
    desc: "Covers the Recharts `[min, max]` tuple parity gap. Data values are `[low, high]` arrays — the area fills between the two bounds.",
    Comp: RangedArea,
    code: rangedAreaCode,
  },
  {
    id: "chart-events",
    group: "Interaction",
    title: "Chart-level events",
    desc: "Covers the Recharts chart-level mouse/touch parity gap. `onChartPointerMove`, `onChartClick`, `onChartPointerDown`, `onChartPointerUp`, and `onChartPointerLeave` fire with SVG coordinates, closest datum index, and visible series values.",
    Comp: ChartEvents,
    code: chartEventsCode,
  },
  {
    id: "per-series-data",
    group: "Data",
    title: "Per-series data",
    desc: "Covers the Recharts per-Series `data` parity gap. Each series can carry its own data array, independent of the chart-level data — here a coffee line reads from chart data while a temperature line reads from its own dataset.",
    Comp: PerSeriesData,
    code: perSeriesDataCode,
  },
  {
    id: "percent-area",
    group: "Stacking",
    title: "Percent-stacked area",
    desc: 'Covers the Recharts `stackOffset="expand"` parity gap. Set `stackOffset="expand"` on `<Chart>` to normalize stacked values to 0–1, then use `tickFormatter` on the Y axis to display percentages.',
    Comp: PercentArea,
    ...
    <div class="min-h-screen bg-zinc-50 text-zinc-900">
      ...
      <section class="mx-auto max-w-5xl px-6 pt-20 pb-12 text-center">
        ...
        <h1 class="mx-auto max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">
          Composable charts, <span class="text-blue-600">built on signals.</span>
        </h1>
        <p class="mx-auto mt-5 max-w-xl text-zinc-500">
          A headless SolidJS charting library — lines, areas, bars, pies, annotations and more,
          every element unstyled and yours to compose.
        </p>
      ...
      <section id="features" class="border-t border-zinc-200 bg-white">
        <div class="mx-auto max-w-5xl px-6 py-16">
          <h2 class="mb-8 text-2xl font-semibold tracking-tight">What makes it peculiar</h2>
...
function FeatureParityRoute() {
  const [id, setId] = createSignal(FEATURE_PARITY_DEMOS[0]!.id);
  const [mode, setMode] = createSignal<"live" | "code">("live");
  const demo = createMemo(
    () => FEATURE_PARITY_DEMOS.find((d) => d.id === id()) ?? FEATURE_PARITY_DEMOS[0]!,
  );

  return (
    <div class="min-h-screen bg-zinc-950 text-zinc-100">
      <main class="mx-auto max-w-5xl px-6 py-10">
        <a href="/" class="text-sm text-zinc-500 transition hover:text-zinc-300">
          peculiar-charts
        </a>
        <div class="mt-12 grid gap-6 border-y border-zinc-800 py-8 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            <p class="font-mono text-xs uppercase tracking-[0.28em] text-zinc-500">
              internal parity tracker
            </p>
            <h1 class="mt-3 max-w-2xl text-4xl font-semibold tracking-tight text-white">
              Feature parity examples, kept off the public demo rail.
            </h1>
            <p class="mt-4 max-w-2xl text-sm leading-6 text-zinc-400">
              This route collects examples for newly closed Recharts parity gaps. The main www page
              stays focused on polished public-facing demos.
            </p>
          </div>
          <code class="rounded-full border border-zinc-800 px-3 py-1 font-mono text-xs text-zinc-500">
            /feature-parity
          </code>
        </div>

        <div class="mt-8 flex flex-wrap gap-2">
          <For each={FEATURE_PARITY_DEMOS}>
            {(d) => (
              <button
                type="button"
                onClick={() => {
                ...
                classList={{
                  "border-zinc-200 bg-zinc-100 text-zinc-950": id() === d.id,
        ...
        <section class="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 shadow-2xl shadow-black/30">
          <div class="mb-4 flex items-start justify-between gap-4">
            <div>
              <p class="font-mono text-xs uppercase tracking-[0.2em] text-zinc-500">
                {demo().group}
              </p>
              <h2 class="mt-1 text-xl font-semibold text-white">{demo().title}</h2>
              <p class="mt-1 max-w-2xl text-sm text-zinc-400">{demo().desc}</p>
            </div>
            <div class="flex shrink-0 rounded-lg border border-zinc-800 p-0.5 text-sm">
              <button
                type="button"
                onClick={() => setMode("live")}
                class="rounded-md px-3 py-1 font-medium transition"
                classList={{
                  "bg-zinc-100 text-zinc-950": mode() === "live",
                  "text-zinc-500": mode() !== "live",
                }}
              >
                Live
              ...
              <button
                ...
                class="rounded-md px-3 py-1 font-mono transition"
                classList={{
                  "bg-zinc-100 text-zinc-950": mode() === "code",
                  "text-zinc-500": mode() !== "code",
                }}
              >
                {"</>"}
          ...
          <Show
            when={mode() === "live"}
            fallback={
              <div class="overflow-auto rounded-xl bg-black p-4 text-[13px] leading-relaxed">
                <pre>
                  <code class="sh" innerHTML={highlight(demo().code)} />
                </pre>
              </div>
          ...
          >
            <div class="relative h-[340px] rounded-xl border border-zinc-800 bg-white p-4 text-zinc-950">
              <Dynamic component={demo().Comp} />
            </div>
          </Show>
        </section>
