...
export default function AxisFormatting() {
  return (
    <Chart<typeof revenue> data={revenue}>
      <Axis axis="y" position="left" tickCount={4} tickFormatter={formatCurrency}>
        <AxisLabel />
      ...
      >
        ...
        <AxisTooltip class="rounded-lg border border-zinc-200 bg-white px-2 py-1 text-xs shadow-lg">
          {(p) => {
            ...
            return (
              <span>
                {formatQuarter(row.quarter)}: <b>{formatCurrency(row.value)}</b>
              </span>
            );
          }}
        </AxisTooltip>
      </Axis>
      <Area dataKey="value" curve={curveNatural} class="text-indigo-200" color="#c7d2fe" />
      <Line
        dataKey="value"
        curve={curveNatural}
