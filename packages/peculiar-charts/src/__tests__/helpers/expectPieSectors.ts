...
export function expectPieSectors(container: Element, expected: ExpectedPieSlice[]): void {
  ...
  expected.forEach((exp, i) => {
    ...
    if (exp.key !== undefined) expect(path).toHaveAttribute("data-key", String(exp.key));
    if (exp.index !== undefined) expect(path).toHaveAttribute("data-index", String(exp.index));
  });
}
