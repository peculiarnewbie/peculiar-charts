...
export type CrosshairProps = Omit<ComponentProps<"line">, "x1" | "y1" | "x2" | "y2">;
...
const Crosshair = (props: CrosshairProps) => {
  ...
  if (isDev && isNumeric(axisContext.scale().type) && axisContext.axis() === "y") {
    throw new Error("[peculiar-charts] Crosshair can not be used with a numeric value axis");
  }
  ...
  const x = () => {
    ...
    return axisContext.axis() === "x" ? tick.position : chartContext.getInset("left");
  };
  const y = () => {
    const tick = closestTick();
    if (!tick) return undefined;
    return axisContext.axis() === "x" ? chartContext.getInset("top") : tick.position;
  };
  const x2 = () => {
    const tick = closestTick();
    if (!tick) return undefined;
  ...
  return (
    <line
      ...
      x2={x2()}
      y2={y2()}
      opacity={chartContext.pointerInChart() || chartContext.syncInteraction()?.active ? 1 : 0}
