...
const Tooltip = (props: TooltipProps) => {
  ...
  if (isDev && isNumeric(axisContext.scale().type) && axisContext.axis() === "y") {
    throw new Error("[peculiar-charts] Tooltip can not be used with a numeric value axis");
  }
  ...
  const payload = createMemo<TooltipPayload | undefined>(() => {
    ...
    return buildTooltipPayload(chartContext, axisContext.axisId(), axisContext.axis(), tick.index);
  });
  ...
  const x = () => {
    ...
    const containerWidth = chartContext.toContainerPosition(chartContext.width(), "width");
    if (axisContext.axis() === "x") {
      const tickPosition = chartContext.toContainerPosition(tick.position, "width");
  ...
  const y = () => {
    ...
    if (!tick) return 0;
    const _tooltipSize = tooltipSize();
    const containerHeight = chartContext.toContainerPosition(chartContext.height(), "height");
    if (axisContext.axis() === "y") {
      const tickPosition = chartContext.toContainerPosition(tick.position, "height");
      const preferred = tickPosition + localProps.tickGap;
