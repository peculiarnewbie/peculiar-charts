...
import { For, Show, createMemo, mergeProps, splitProps, type ComponentProps } from "solid-js";
...
const ringPath = (cx: number, cy: number, radius: number, angles: number[]) => {
...
const PolarGrid = (props: PolarGridProps) => {
  const [localProps, otherProps] = splitProps(
    mergeProps({ angleAxisId: "angle", gridType: "polygon" as const, radialLines: true }, props),
  ...
  return (
    <g data-pc-polar-grid-group="" {...otherProps}>
      <Show when={localProps.radialLines}>
        <For each={angles()}>
          {(angle) => <path d={spokePath(angle)} fill="none" data-pc-polar-grid="" />}
        </For>
      </Show>
      <For each={ringRadii()}>
        {(r) =>
          localProps.gridType === "circle" ? (
            <circle cx={layout.cx()} cy={layout.cy()} r={r} fill="none" data-pc-polar-grid="" />
          ) : (
            <path
