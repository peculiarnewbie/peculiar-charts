import { type BrushRange, useChartContext } from "@src/components/context";
...
const Brush = (props: BrushProps) => {
  ...
  const end = createMemo(() => (local.endIndex !== undefined ? local.endIndex : internalEnd()));

  const clampedStart = createMemo(() => Math.max(0, Math.min(start(), dataLen() - 1)));
  const clampedEnd = createMemo(() => Math.max(clampedStart(), Math.min(end(), dataLen() - 1)));
  ...
  const handlePointerDown = (which: "start" | "end" | "slide", e: MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (which === "start") {
        const newStart = Math.max(0, Math.min(dragStartIdx() + indexDelta, clampedEnd()));
        setRange(newStart, clampedEnd());
      } else if (which === "end") {
        const newEnd = Math.max(clampedStart(), Math.min(dragEndIdx() + indexDelta, len - 1));
    ...
    <g data-pc-brush="" {...other}>
      ...
      <Show when={local.children}>
        <clipPath id={`pc-brush-clip-${clipId}`}>
          <rect x={pad} y={brushY() + pad} width={innerW()} height={innerH()} />
        </clipPath>
        <g clip-path={`url(#pc-brush-clip-${clipId})`}>
          <BrushContextProvider mainContext={ctx} width={innerW} height={innerH} data={ctx.data}>
            <g transform={`translate(${pad}, ${brushY() + pad})`}>{local.children}</g>
