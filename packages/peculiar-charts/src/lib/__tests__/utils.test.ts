...
import { accessData, gapToPadding, pointDefined, toNumeric, uniqueInOrder } from "../utils";
...
describe("accessData", () => {
  ...
  it("extracts values by dot-path key", () => {
    const data = [{ stats: { count: 1 } }, { stats: { count: 2 } }];
  ...
  it("throws when dataKey is set but data is not an array", () => {
    expect(() => accessData("not-an-array" as any, "key")).toThrow(/Expected data to be an array/);
  });

  it("throws when no dataKey and data is not an array", () => {
    expect(() => accessData(null as any, undefined)).toThrow(/Expected data to be an array/);
  });
});
