...
export const createPolarRadiusScale = (props: {
  ...
  createMemo(() => {
    const domain = props.chartContext.getDomain(props.axisId(), "radius");
    ...
    const scale = scaleLinear([min, max], [props.layout.innerRadius(), props.layout.outerRadius()]);
...
export const projectRadiusScale = (scale: PolarRadiusScale, value: number): number => {
