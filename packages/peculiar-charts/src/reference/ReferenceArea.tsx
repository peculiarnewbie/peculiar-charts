...
const ReferenceArea = (props: ReferenceAreaProps) => {
  ...
  const geom = createMemo(() => {
    ...
    const px1 =
      localProps.x1 !== undefined ? projectScale(xScale(), localProps.x1) : left
    const px2 =
      localProps.x2 !== undefined
        ? projectScale(xScale(), localProps.x2)
        : right
    const py1 =
      localProps.y1 !== undefined
        ? projectScale(yScale(), localProps.y1)
        : bottom
    const py2 =
      localProps.y2 !== undefined ? projectScale(yScale(), localProps.y2) : top
    return {
      x: Math.min(px1, px2),
      y: Math.min(py1, py2),
